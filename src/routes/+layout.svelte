<script lang="ts">
	...
	//TODO: use native Temporal API when it's widely supported
	//Note: 04/02/2026 Temporal is in active adaptation stage, already supported by Chrome and Edge
	// but not yet supported by Safari, so we use the polyfill for now
			...
			>
				...
				>
					...
					<a
						...
						target="_blank"
						rel="noopener noreferrer"
						class="h-5 w-5 nav-link-subtle grayscale hover:grayscale-0"
						aria-label="GitHub"
					>
						<!-- hover 灰度-强调；svg 调用，viewbox 配置使之适合 20x20 -->
						<img src={GithubIcon} alt="GitHub" class="h-5 w-5 grayscale hover:grayscale-0" />
						<svg
